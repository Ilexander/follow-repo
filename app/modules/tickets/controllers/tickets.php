...
class tickets extends My_UserController
{
    ...
    public function index()
    {
        ...
        if ($supportStatus->conn_id->affected_rows > 0) {
            $adminOnline = $supportStatus->row()->value;
        }
        ...
        if (!$supportOnline) {
            if ($dt->format('Hi') < $supportStart) {
                ...
                $date = DateTime::createFromFormat("Y-m-d H:i", $dt->format('Y-m-d').' 10:00');
                $interval = $dt->diff($date);

                $timeLeft = ( $interval->h - 3 ) * 100 + $interval->i;

            } else {

                $date = DateTime::createFromFormat("Y-m-d H:i", $dt->format('Y-m-d').' 23:59');
                $interval = $dt->diff($date);
                $timeLeft = ( $interval->h - 3 ) * 100 + $interval->i + $supportStart;
    ...
    public function store()
    {
        ...
        $default_ticket_per_user = get_option('default_ticket_per_user', 5);
        if ($items_pending_number > $default_ticket_per_user && $default_ticket_per_user != 0) {
