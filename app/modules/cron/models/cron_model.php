...
class cron_model extends MY_Model {
    ...
    public function get_item($params = null, $option = null)
    {
        ...
        if ($option['task'] == 'get-item-provider') {
            $result = $this->get('url, key, type, id, token', $this->tb_api_providers, ['id' => $params['id']], '', '', true);
        }
        return $result;
    }
    ...
    public function save_item($params = null, $option = null)
    {
        if ($option['task'] == 'item-new-update') {
            ...
            if (!$params['response']) {
                ...
                $this->db->select("*");
                $this->db->from($this->tb_main);
                $this->db->where('id', $params['order_id']);
                $query = $this->db->get();
                $order = $query->result();
                ...
                if (!empty($order)) {
                    ...
                    if (!empty($user)) {
                        ...
                        $this->db->update($this->tb_users, ["balance" => $balance], ["id"=> $user->uid]);
                    }
                }
        ...
        if ($option['task'] == 'item-multiple_status') {
            if (isset($params['response']['error'])) {
                ...
                $this->db->select("*");
                $this->db->from($this->tb_main);
                $this->db->where('id', $params['order_ids']);
                $query = $this->db->get();
                $orders = $query->result();
